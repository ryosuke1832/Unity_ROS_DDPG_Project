...
public class DeformableTarget : MonoBehaviour
{
    ...
    /// <summary>
    /// 外部から把持開始を通知（BeginGraspingとして追加）
    /// </summary>
    /// <param name="force">把持力（N）</param>
    /// <param name="graspPosition">把持位置</param>
    /// <param name="graspDir">把持方向</param>
    public void BeginGrasping(float force, Vector3 graspPosition, Vector3 graspDir)
    {
        currentForce = force;
        graspPoint = graspPosition;
        graspDirection = graspDir;
        isBeingGrasped = true;
        
        if (showDebugInfo)
        {
            Debug.Log($"把持開始 - 力: {force}N, 位置: {graspPosition}, 方向: {graspDir}");
        }
    }
    ...
    /// <param name="graspPosition">把持位置</param>
    /// <param name="graspDir">把持方向</param>
    public void StartGrasping(float force, Vector3 graspPosition, Vector3 graspDir)
    {
        currentForce = force;
        graspPoint = graspPosition;
        graspDirection = graspDir;
    ...
    /// 把持力を更新
    /// </summary>
    /// <param name="force">新しい把持力</param>
    public void UpdateGraspForce(float force)
    {
        if (isBeingGrasped)
        {
            currentForce = force;
    ...
    /// 現在の変形状態を取得
    /// </summary>
    /// <returns>変形状態情報</returns>
    public DeformationState GetDeformationState()
    {
        return new DeformationState
        {
            isBeingGrasped = this.isBeingGrasped,
            currentForce = this.currentForce,
            currentDeformation = this.currentDeformation,
            deformationType = this.deformationType,
            softness = this.softness,
            isDeformed = this.IsDeformed
        };
    }
