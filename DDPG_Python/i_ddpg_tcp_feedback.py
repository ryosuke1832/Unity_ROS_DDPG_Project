#!/usr/bin/env python3
"""
TypeA DDPG学習システム（修正版・実験計画準拠）

修正点：
1. K=1設計: 同一エピソード内で(s,a,r,s',done)を確定
2. pending方式: アクション送信直前の状態を保持
3. 対称正規化: F中心11.5N、半幅3.5Nで[-1,1]正規化
4. 自動指標計算: AUC/plateau/time-to-70/gap-to-plateau
5. 5シード対応: 複数実行での統計計算

実験タイプ：
- A_400: 400エピソード（TypeBとの公平比較用）
- A_long: 5000エピソード（到達上限・plateau測定用）
"""
...
class TypeADDPGSystem:
    ...
    def __init__(self, experiment_type="A_400", seed=42):
        ...
        # TCP通信
        self.tcp_interface = EEGTCPInterface(host='127.0.0.1', port=12346)
        ...
        print(f"🤖 TypeA DDPG学習システム初期化完了（修正版）")
    ...
    def handle_grip_force_request(self, message_data):
        """Unityからの把持力リクエスト処理（Pending方式）"""
        try:
            if message_data.get('type') == 'grip_force_request':
                # 現在の状態を取得（前回のアクションは保持済み）
                current_tcp_data = message_data
                prev_action = 0.0 if self.pending_action is None else self.pending_action[0]
                
                # 状態作成
                state = self.create_state(current_tcp_data, prev_action)
                ...
                print(f"🎯 TypeA把持力応答: {grip_force:.2f}N (action: {action[0]:.3f})")
    ...
    def _learning_loop(self):
        """学習ループ（Pending方式）"""
        print(f"🔄 TypeA学習ループ開始（Pending方式）")
    ...
    def _print_learning_progress(self):
        ...
        print(f"\n📊 TypeA進捗 (Episode {self.episode_count}/{self.target_episodes}):")
    ...
    def _save_model(self):
        """モデル保存"""
        try:
            model_path = os.path.join(self.output_dir, f'typea_model_ep{self.episode_count}.pth')
            torch.save({
...
def main():
    """メイン実行関数"""
    parser = argparse.ArgumentParser(description="TypeA DDPG学習システム（修正版）")
